...
let lolsubs = ["Before School", "Recess", "Lunch", "Home", "temp", "Remove me"];
...
function updateTimetable() {
  ...
  const week = 0;
  let trueDate = now.getDay() + week * 7;
  // trueDate = 6;
  const tomorrow = (trueDate + 1) % 7;
  ...
  if (subsAfter.map(s => s.name).filter(n => lolsubs.indexOf(n) == -1).length == 0) {
    ...
    document.getElementById("afterSub").innerText = subsAfter
      .map((s,i,a) => {
        if (a?.[i+1]?.name === s.name) {
          return "Double "+s.name
        } else if (a?.[i-1]?.name === s.name) {
          return "Remove me"
        } {
          return s.name
        }
...
for (weekID of ["A"]) {
  for (day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]) {
    ...
    for (period of periods) {
      ...
      //   input.disabled = true;
      // }
      if (day%2==0 && day > 0 && period == 6) {
        inputHolder.classList.add("invisible");
      }
      let val = `sub${
